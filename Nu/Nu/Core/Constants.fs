...
module Render =
    ...
    let [<Literal>] MorphsMax = 16 // NOTE: max number of morphs active at one time, not max number of morph targets in total.
    let [<Literal>] BonesMax = 170 // NOTE: remember to update BONES_MAX in shaders when changing this!
    ...
    let [<Literal>] InstanceFieldCount = 41 // NOTE: remember to add VertexArrayAttribFormat specifications when extending this! NOTE: two fields free for additional parameters.
