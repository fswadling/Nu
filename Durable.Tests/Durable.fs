...
module Durable =
    ...
    let rec mapEffect
        ...
        match durable with
        | Return result -> Return result
        | Yield (effect, next) -> Yield (mapper effect, mapEffect mapper next)
        | Suspend continuation ->
    ...
    let rec foldEffect
        ...
        | Yield (effect, next) ->
            let newSeed = folder seed effect
            Yield (newSeed, foldEffect newSeed folder next)
        | Suspend continuation ->
            let newContinuation event =
                let durable = continuation event
                foldEffect seed folder durable
            Suspend newContinuation
    ...
    /// Yield an effect and then suspend waiting for an event
    let repeat (effect: 'TEffect) =
        let rec loop () = Yield (effect, Suspend (fun _ -> loop ()))
        loop ()
    ...
    let rec toEffect (durable: Durable<'TEvent, 'TEffect, 'TResult>) : 'TEffect seq =
        match durable with
        | Suspend _ -> Seq.empty
        | Yield (effect, next) -> seq { yield effect; yield! toEffect next }
        | Return _ -> Seq.empty
