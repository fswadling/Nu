...
type MMCCGameCommand =
    | Exit
    interface Command

// this extends the Game API to expose the above MMCC model as a property.
[<AutoOpen>]
module MMCCGameExtensions =
    type Game with
        member this.GetMMCCGame world = this.GetModelGeneric<MMCCGame> world
        member this.SetMMCCGame value world = this.SetModelGeneric<MMCCGame> value world
        member this.MMCCGame = this.ModelGeneric<MMCCGame> ()
...
type MMCCGameDispatcher () =
    inherit GameDispatcher<MMCCGame, MMCCGameMessage, MMCCGameCommand> (Splash)

    // here we define the game's properties and event handling
    override this.Definitions (mMCCGame, _) =
        [Game.DesiredScreen :=
            match mMCCGame with
         ...
         if mMCCGame = Splash then Simulants.Splash.DeselectingEvent => ShowTitle
