...
[<AutoOpen>]
module MyGame3Extensions =
    type Game with
        member this.GetMyGame3 world = this.GetModelGeneric<MyGame3> world
        member this.SetMyGame3 value world = this.SetModelGeneric<MyGame3> value world
        member this.MyGame3 = this.ModelGeneric<MyGame3> ()
...
type MyGame3Dispatcher () =
    inherit GameDispatcher<MyGame3, MyGame3Message, MyGame3Command> (Splash)

    // here we define the game's properties and event handling
    override this.Definitions (myGame3, _) =
        [Game.DesiredScreen :=
            match myGame3 with
         ...
         if myGame3 = Splash then Simulants.Splash.DeselectingEvent => ShowTitle
