...
module StateMachine =
    ...
    let rec until
        (predicate: 'TEvent -> bool)
        (state: 'TState)
        : StateMachine<'TEvent,'TState,unit> =

        let continuation event =
            if predicate event
            then Return ()
            else until predicate state
    ...
    let rec untilChoose
        ...
        let continuation event =
            chooser event
            |> Option.map Return
            |> Option.defaultValue (untilChoose chooser state)
    ...
    let rec foldState
        ...
        let state = folder seed state

        let continuation event =
            let stateMachine = continuation event
            foldState state folder stateMachine
