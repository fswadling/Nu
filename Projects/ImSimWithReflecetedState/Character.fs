...
module CharacterProp =
    let doEntity (name: string) (prop: CharacterProp) (world: World) =
    ...
    let cameraFollow (world: World) (prop: CharacterProp) =
        let cameraRotation = prop.Rotation * Quaternion.CreateFromAxisAngle (v3Up, float32 Math.PI_MINUS_EPSILON)
        do World.setEye3dCenter (prop.Position + v3Up * 1.40f - cameraRotation.Forward) world
